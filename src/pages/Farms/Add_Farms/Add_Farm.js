...
const Add_areas = () => {
  ...
  const Fetchdata = () => {
    Service.get("areas").then((res) => setarea_id_all(res.records));
  };
  useEffect(() => {
    Fetchdata();
  }, []);
...
></span> */}
          <div className={Style.close}>
            <CloseOutlined onClick={(e) => navigate("/dashoard/All_Farms")} />
          </div>
          <h1>Create Farm</h1>
          <form
            onSubmit={handleSubmit((data) =>
              // console.log("data",postObject)
              Service.post("create/farms", postObject).then((res) => {
          ...
          >
            ...
            <Form.Item name="Farm" required label={<span>Area&nbsp;</span>}>
              <Select
                showSearch
                placeholder="Select a Farm"
                optionFilterProp="children"
                onChange={(e) => setPostObject({ ...postObject, area_id: e })}
